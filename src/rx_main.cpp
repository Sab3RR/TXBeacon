...
uint32_t cycleReq;
uint32_t cycleMax = 0x0FFFFFFF;
uint32_t cycleMin = 0;
uint32_t cycleAver;
uint32_t const offSet = 250;
...
std::vector<double> rangeTime;
std::vector<uint32_t> rangeArray;


double kalman(double U)
{
    static const double R = 5000;
    static const double H = 1.00;
    static double Q = 10;
    static double P = 0;
    static double U_hat = 520000;
    static double K = 0;
...
//        Serial.write(str, l);
    ...
    if (otaPktPtr->std.type == PACKET_TYPE_SYNC)
    {
        ...
        if (responce)
        {
            res = Radio.CallBack32 - cycleReq;

        
            if (cycleMin < offSet)
                cycleMin = offSet;
            if (cycleMax + offSet < cycleMax)
                cycleMax = cycleMax - (cycleMax + offSet + 1);
            if (res > cycleMin - offSet  && res < cycleMax + offSet)
            {
                rangeTime.push_back(micros64() / (double)1000000);
                rangeArray.push_back((uint32_t)kalman((double)res));
            }
        }
        ...
        // char str[20];
        // int l = sprintf(str, "range = %i\n", rangeArray.size());
        // Serial.write(str, l);
//        str = (char*)malloc(1000);
//        int l = sprintf(str, "OtaPacket\n type = %x\n crcHigh = %x\n crcLow = %x\n fhssIndex = %i\n nonce = %x\n UID3 = %i\n UID4 = %i\n UID5 = %i\n reqTime = %lu\n cycletime = %lu\n reqtime = %lu\n",
//                            otaPktPtr->std.type, otaPktPtr->std.crcHigh, otaPktPtr->std.crcLow, otaPktPtr->std.sync.fhssIndex, otaPktPtr->std.sync.nonce, otaPktPtr->std.sync.UID3, 
//                            otaPktPtr->std.sync.UID4, otaPktPtr->std.sync.UID5, res, clockcallback, cycleReq);
    //    Serial.write(str, l);
//        free(str);
        responce = false;
        request = true;
    ...
    else if (otaPktPtr->std.type == PACKET_TYPE_MSPDATA)
    {
        ...
        if (otaPktPtr->msp.msp_ul.payload.type == TYPE_SERVICE_TO_SYNC_RESPONCE && otaPktPtr->msp.msp_ul.payload.data.service_to_sync_responce.id == command.get_syncRespId())
        {
            if (otaPktPtr->msp.msp_ul.payload.data.service_to_sync_responce.key8 == KEY8 && otaPktPtr->msp.msp_ul.payload.data.service_to_sync_responce.key16 == KEY16)
            {
        ...
        else if (otaPktPtr->msp.msp_ul.payload.type == TYPE_GPS_RESPONCE)
        {
            ...
            {
            case 0:
                command.set_1lat(otaPktPtr->msp.msp_ul.payload.data.gps_responce.responce); 
                break;
            case 1:
                command.set_2lat(otaPktPtr->msp.msp_ul.payload.data.gps_responce.responce);  
                break;
            case 2:
                command.set_1lng(otaPktPtr->msp.msp_ul.payload.data.gps_responce.responce); 
                break;
            case 3:
                command.set_2lng(otaPktPtr->msp.msp_ul.payload.data.gps_responce.responce); 
                break;
            case 4:
                command.set_1alt(otaPktPtr->msp.msp_ul.payload.data.gps_responce.responce);
                break;
            case 5:
                command.set_2alt(otaPktPtr->msp.msp_ul.payload.data.gps_responce.responce);                 
                break;
            }
            command.set_grResp(true);
            command.inc_gpsIter();
            char str[100];
            int l = sprintf(str, "GPS\n Packet = %u\n responce = %lu\n lat = %f\n lng = %f\n alt = %f\n", otaPktPtr->msp.msp_ul.packageIndex, otaPktPtr->msp.msp_ul.payload.data.gps_responce.responce, command.get_lat(), command.get_lng(), command.get_alt());
            Serial.write(str, l);
        }
...
//    }
    ...
    //    int l = sprintf(str, "size = %i, max = %lu, min = %lu", rangeArray.size(), cycleMax, cycleMin);
    //    Serial.write(str, l);
